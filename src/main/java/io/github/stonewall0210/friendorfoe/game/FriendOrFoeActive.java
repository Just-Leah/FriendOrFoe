...
public class FriendOrFoeActive {
    private final FriendOrFoeConfig config;

    public final GameSpace gameSpace;
    private final FriendOrFoeMap gameMap;

    // TODO replace with ServerPlayerEntity if players are removed upon leaving
    private final Object2ObjectMap<PlayerRef, FriendOrFoePlayer> participants;
    private final FriendOrFoeSpawnLogic spawnLogic;
    private final FriendOrFoeStageManager stageManager;
    private final boolean ignoreWinState;
    private final FriendOrFoeTimerBar timerBar;

    private FriendOrFoeActive(GameSpace gameSpace, FriendOrFoeMap map, GlobalWidgets widgets, FriendOrFoeConfig config, Set<PlayerRef> participants) {
        ...
        for (PlayerRef player : participants) {
            this.participants.put(player, new FriendOrFoePlayer());
        }

        this.stageManager = new FriendOrFoeStageManager();
        this.ignoreWinState = this.participants.size() <= 1;
        this.timerBar = new FriendOrFoeTimerBar(widgets);
    }

    public static void open(GameSpace gameSpace, FriendOrFoeMap map, FriendOrFoeConfig config) {
        gameSpace.openGame(game -> {
            Set<PlayerRef> participants = gameSpace.getPlayers().stream()
                    .map(PlayerRef::of)
                    .collect(Collectors.toSet());
            GlobalWidgets widgets = new GlobalWidgets(game);
            FriendOrFoeActive active = new FriendOrFoeActive(gameSpace, map, widgets, config, participants);
    ...
    private void tick() {
        ...
        FriendOrFoeStageManager.IdleTickResult result = this.stageManager.tick(time, gameSpace);
